defmodule Needle.UID do
  ...
  def generate(schema) when is_atom(schema) do
    ...
    if function_exported?(schema, :__schema__, 1) do
      case schema.__schema__(:primary_key) |> List.first() |> debug( "gen primary_key first") do # hopefully ok to just take the first?
  ...
  def cast!(value, params \\ nil) do
    case cast(value, params) do
      {:ok, uid} -> uid
      {:error, term} -> raise Ecto.CastError, type: __MODULE__, value: value, message: term[:message]
